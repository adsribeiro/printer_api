...
- [ ] **Suporte a PDFs Reais**: Implementar a impressão direta de arquivos PDF existentes (utilizando `Ghostscript` ou `PDFtoPrinter.exe`).
- [ ] **Formatação de Texto (Comum)**: Adicionar suporte a negrito, tamanhos de fonte variáveis e alinhamento no modo de impressão comum.
- [ ] **Fila de Impressão Interna**: Usar `BackgroundTasks` do FastAPI para que a API responda imediatamente enquanto a impressão ocorre de forma assíncrona.

## 📊 Fase 3: Monitoramento e Interface (Médio Prazo)
- [ ] **Painel Administrativo (UI)**: Uma pequena interface web integrada (FastAPI + Jinja2) para visualizar o status das impressoras e logs recentes.
- [ ] **Monitor de Status da Fila**: Integrar com a API do Windows para retornar o status real do trabalho de impressão (ex: `Printing`, `Paused`, `Error`).
- [ ] **Autenticação**: Implementar `API Key` simples para evitar que qualquer pessoa na rede dispare impressões indesejadas.

## 🌐 Fase 4: Escalabilidade e Ecossistema (Longo Prazo)
